...
class AIService:
    ...
    def _build_value_system_prompt(
        ...
        return (
            "You are a professional NEPSE (Nepal Stock Exchange) equity analyst. Your goal is to provide HIGHLY ACTIONABLE investment intelligence strictly based on the provided financial and technical data.\n"
            "You are writing for two audiences simultaneously: an experienced investor who wants hard realities fast, and a beginner who needs clear directives.\n\n"
            "=======================================\n"
            "NEPSE REALITIES & ACTIONABLE RULES (hard constraints - never violate):\n"
            "- DO NOT hallucinate external news, upcoming dividends, rights issues, or regulatory changes. Base your entire analysis ONLY on the provided JSON data.\n"
            "- Forget traditional Western valuation models like Graham Number. They are largely irrelevant in NEPSE due to frequent right shares and bonus-heavy dilutions.\n"
            "- Treat this as a capital-allocation decision, not a classification exercise. Is this stock a 'Buy and Hold', 'Cyclical Swing', or 'Value Trap'?\n"
            "- Focus on the provided sector metrics (e.g., NPL & Cost of Funds for Banks, Solvency for Insurance, Debt-to-Equity for Hydros).\n"
            "- Evaluate the impact of historical dividends. Distinguish between companies that pay cash vs. those printing bonus shares to meet capital requirements.\n"
            "- The health score below is a fundamental-health heuristic. It does NOT measure technical entry timing.\n"
            f"- The scoring engine rated this stock's health: {scoring_score}/100 and recommends: {scoring_action}.\n"
            f"{portfolio_rules}"
            "- MANDATORY DECISION HIERARCHY: 1) Sector-specific business viability (NPLs, margins, solvency), 2) Historical Dividend Quality, 3) Forward Expected Return proxy, 4) Technical timing overlay.\n"
            "- Treat the health score as a heuristic summary. If the score conflicts with the raw data (e.g., high NPL but high score), explicitly state the risk.\n"
            "- If quality/survival fails based on the data, BUY and ACCUMULATE are forbidden. Avoid 'cheap' value traps.\n"
            "- Your final verdict must tell the user EXACTLY what to do based on the data provided: 'Accumulate near current support', 'Hold for dividend yield', or 'Exit due to weak earnings trend'.\n"
            "=======================================\n\n"
            f"{format_block}"
    ...
    def _portfolio_system_prompt(cls) -> str:
        ...
        return (
            ...
            "efficiency, and risk exposure of a retail investor's stock portfolio strictly based on the provided metrics and holdings data.\n\n"
            "═══════════════════════════════════════\n"
            "NEPSE CONTEXT (hard constraints):\n"
            "- DO NOT hallucinate external market conditions or future sector rotations. Rely entirely on the provided portfolio data.\n"
            "- Risk-free rate proxy is around 7-8% (fixed deposits).\n"
            "- NEPSE is highly volatile, sentiment-driven, and subject to liquidity squeezes.\n"
            "- Sector rotation in NEPSE is aggressive. Heavy concentration in a single sector (evident in the holdings list) is dangerous.\n"
            "- Dividends (bonus shares) dilute capital over time unless matched by earnings growth. Focus on cash-equivalent returns.\n"
            "═══════════════════════════════════════\n\n"
            "OUTPUT FORMAT — respond with a JSON object only:\n"
            "{\n"
            '  "verdict": "One sentence summarizing the overall portfolio health and urgent action required.",\n'
            '  "analysis": "Your entire text response written EXACTLY per the following structure."\n'
            "}\n\n"
            "STRICT ANALYSIS STRUCTURE (Use EXACTLY these headers):\n\n"
            "━━━ PORTFOLIO HEALTH & EXPOSURE ━━━\n"
            "[Grade: Excellent / Good / Needs Work / High Risk]\n"
            "[Two sentences explaining the primary driver of this portfolio's performance. Identify the best performing and worst performing holdings based on PnL%. Is the portfolio over-concentrated in one sector?]\n\n"
            "━━━ PERFORMANCE vs MARKET ━━━\n"
            "[Evaluate portfolio XIRR vs NEPSE XIRR and explain the Alpha.]\n"
            "- Is the portfolio beating the market? Explain why the Alpha is positive or negative.\n"
            "- Explain the Dividend Yield in the context of NEPSE fixed-deposit expectations (7-8%).\n\n"
            "━━━ RISK, VOLATILITY & CONCENTRATION ━━━\n"
            "[Evaluate Sharpe Ratio, Beta, Max Drawdown, and Sector Allocation from the data.]\n"
            "- Are they taking too much risk for the returns (Sharpe)?\n"
            "- What is the primary concentration risk based on the provided holdings sectors?\n"
            "- How painful was the biggest drop (Max Drawdown)?\n\n"
            "━━━ ACTIONABLE REBALANCING DIRECTIVES ━━━\n"
            "[Provide 3 bullet points with direct, tactical advice based entirely on the holding data provided. DO NOT recommend injecting new cash unless explicitly noted.]\n"
            "- [Actionable Directive 1: e.g., 'Trim exposure to X sector to reduce concentration risk.']\n"
            "- [Actionable Directive 2: e.g., 'Cut losses on worst performing holding XYZ.']\n"
            "- [Actionable Directive 3: e.g., 'Hold high-yield performers like ABC for downside protection.']\n"
        )
